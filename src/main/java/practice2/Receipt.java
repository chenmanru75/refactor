...
//    }
    private BigDecimal calculateSubtotal(List<Product> products, List<OrderItem> items) {
        ...
        for (Product product : products) {
            OrderItem item = findOrderItemByProduct(items, product);
            BigDecimal itemTotal = calculatePriceMultCount(product, item);
            BigDecimal reducedPrice = itemTotal.multiply(product.getDiscountRate());
            subTotal = subTotal.add(itemTotal).subtract(reducedPrice);
        }
        return subTotal;
    }

    private BigDecimal calculatePriceMultCount(Product product, OrderItem item) {
